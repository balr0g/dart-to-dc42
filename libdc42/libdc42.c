...
int dc42_open(DC42ImageType *F, char *filename, char *options)
{
    ...
    if (filesizetotal>(1024*1024*64)) DC42_RET_CODE(F,88,"File too large to be a valid disk image",return F->retval);
...
int dc42_create(char *filename, char *volname, uint32 datasize, uint32 tagsize)   // create a new disk image
{
  ...
  memset(pascalstring, 0, 64);            // clear pstr for consistency
  ...
  if (datasize==800*512   && tagsize==800*12)   {dskformat=0; formatbyte=0x02;}
  if (datasize==1600*512  && tagsize==1600*12)   dskformat=1;

  if (datasize==800*512   && tagsize==0)        {dskformat=0; formatbyte=0x02;}
